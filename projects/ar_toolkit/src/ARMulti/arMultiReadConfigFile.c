...
static char *get_sbuff( char *buf, int n, char **config );
...
ARMultiMarkerInfoT *arMultiReadConfigFile( const char *filename, ARPattHandle *pattHandle )
{
    ...
    for( i = 0; i < num; i++ ) {
        get_buff(buf, 256, fp);
        if (sscanf(buf, "%llu", &(marker[i].globalID)) != 1) { // Try first as matrix code.
...
ARMultiMarkerInfoT *arMultiLoadConfigString( char *config, ARPattHandle *pattHandle )
{
    ARMultiEachMarkerInfoT *marker;
    ARMultiMarkerInfoT     *marker_info;
    ARdouble               wpos3d[4][2];
