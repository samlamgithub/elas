                     ...
                     const AR_MATRIX_CODE_TYPE matrixCodeType )
{
    ...
    for( i = j = 0; i < marker2_num; i++ ) {
        ...
        // Added to calculate the point without distortion
        if( arGetRawLine(markerInfo2[i].x_coord, markerInfo2[i].y_coord, markerInfo2[i].coord_num,
                      markerInfo2[i].vertex, arParamLTf, markerInfo[j].line, markerInfo[j].raw_vertex) < 0 ) continue;

        if( arGetLine(markerInfo2[i].x_coord, markerInfo2[i].y_coord, markerInfo2[i].coord_num,
                      markerInfo2[i].vertex, arParamLTf, markerInfo[j].line, markerInfo[j].vertex) < 0 ) continue;
