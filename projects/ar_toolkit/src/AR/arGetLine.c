...
int arGetRawLine(int x_coord[], int y_coord[], int coord_num, int vertex[], ARParamLTf *paramLTf,
              ARdouble line[4][3], ARdouble v[4][2])
{
    ARMat    *input, *evec;
    ARVec    *ev, *mean;
    ...
    for( i = 0; i < 4; i++ ) {
        ...
        for( j = 0; j < n; j++ ) {
#ifdef ARDOUBLE_IS_FLOAT
            input->m[j*2+0] = (float)x_coord[st+j];
            input->m[j*2+1] = (float)y_coord[st+j];
#else
            input->m[j*2+0] = (ARdouble)x_coord[st+j];
            input->m[j*2+1] = (ARdouble)y_coord[st+j];
#endif
        }
        if( arMatrixPCA(input, evec, ev, mean) < 0 ) goto bail;
        line[i][0] =  evec->m[1];
